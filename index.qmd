...
author: "Mahlaku Sebiloane " # Remember to add your name!
date: "16-10-2024"
theme: "yeti"
---
...
# Create table
 cantril_wider <-  pivot_wider(cantril, names_from = Year, values_from = `Cantril ladder score`, id_cols = Country)


hap_table <- cantril_wider %>% 
  select(Country, `2011`, `2023`) %>% 
  mutate(absolute_change = (`2023` - `2011`),
...
# Create line chart
cantril_filter <- cantril %>% 
  filter(Country %in% c("South Africa", "China", "Nigeria", "Ethiopia", "Switzerland"))

hap_plot <- ggplot(cantril_filter, aes(x = Year, y = `Cantril ladder score`, col = Country)) +
  geom_line() +
  scale_y_continuous(limits = c(0, 10)) +
  labs(title = "Line Graph showing happiness for selected countries", x = "Year", 
